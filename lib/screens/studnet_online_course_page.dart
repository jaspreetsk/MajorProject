...
Future<void> _uploadOnlineCourseData({
  ...
  try {
    ...
    String docId = courseName.trim(); // Use course name as document ID - or generate unique ID if needed.
    ...
    if (proofDocumentUrls != null && proofDocumentUrls.isNotEmpty) { // Check if list is not null and not empty
      courseData['proofDocumentUrl'] = proofDocumentUrls; // Store the list
    }
    if (certificateUrls != null && certificateUrls.isNotEmpty) { // Check if list is not null and not empty
    ...
    await courseCollection.doc(docId).set(courseData); // Use course name as doc ID
    ...
    print('Error uploading online course data: $e'); // Log error for debugging
  }
}
    ...
    builder: (BuildContext context,
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          String platformName = data['platformName'] ?? 'No Platform Specified';
          String duration = data['duration'] ?? 'N/A';
          List<dynamic> certificateUrlsDynamic = data['certificateUrl'] ?? [];
          List<String> certificateUrls = certificateUrlsDynamic.cast<String>().toList();
          List<dynamic> proofDocumentUrlsDynamic = data['proofDocumentUrl'] ?? [];
          List<String> proofDocumentUrls = proofDocumentUrlsDynamic.cast<String>().toList();
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        if (skillsAcquired.isNotEmpty && onlineCourseType == 'past') // Show skills for past courses only
                          Text(
                            'Skills Acquired: $skillsAcquired',
                            style: const TextStyle(
                                fontSize: 16, color: Color(0xFF6C7F93)),
                          ),
                        if (certificateUrls.isNotEmpty && onlineCourseType == 'past')
                          Padding(
                            padding: const EdgeInsets.only(top: 8.0),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                          ...
                          if (proofDocumentUrls.isNotEmpty && onlineCourseType == 'current')
                            Padding(
                              padding: const EdgeInsets.only(top: 8.0),
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (studentId != null) {
                                      List<String> allMediaUrls = [
                                        ...certificateUrls.map((url) => url.toString()),
                                        ...proofDocumentUrls.map((url) => url.toString()),
                                      ];
                                      await _deleteOnlineCourse( // Create this function next
                                        studentId: studentId,
  ...
  void _loadOnlineCourse() {
    ...
    if (studentId != null) {
      ...
      _pastOnlineCourseStream = // Use _pastInternshipStream
          _getOnlineCourseStream(studentId, 'past'); // Call _getInternshipStream
    }
  }
...
Future<void> _deleteOnlineCourse({
  ...
  List<String>? mediaUrls, // Changed from fileUrls to mediaUrls to handle both proofs and certificates
}) async {
  try {
      ...
      for (String mediaUrl in mediaUrls) {
        ...
        if (mediaUrl.isNotEmpty) {
          ...
          try {
            ...
            print('Error deleting file from storage: $mediaUrl - $storageError');
          }
        }
      }
    }
    ...
    await firestore
        .collection('online_courses') // Changed collection to 'online_courses'
        .doc(studentId)
    ...
    showSnackBar(
        context, 'Error deleting online course details: $e'); // Updated message
    print('Error deleting online course data: $e'); // Log error for debugging
  }
}

void _showEditOnlineCourseDialog(BuildContext context,
    DocumentSnapshot<Map<String, dynamic>> document, String onlineCourseType) {
  ...
  List<dynamic> currentCertificateUrlsDynamic = document.data()?['certificateUrl'] ?? [];
  List<dynamic> currentProofDocumentUrlsDynamic = document.data()?['proofDocumentUrl'] ?? [];

  List<String> currentCertificateUrls = currentCertificateUrlsDynamic.cast<String>().toList();
  List<String> currentProofDocumentUrls = currentProofDocumentUrlsDynamic.cast<String>().toList();
  ...
  List<String> certificateFileNames = [...currentCertificateUrls]; // Initially URLs, will be replaced by filenames after upload
  List<String> proofDocumentFileNames = [...currentProofDocumentUrls]; // Initially URLs, will be replaced by filenames after upload

  List<File> newCertificateFiles = []; // To hold newly selected certificate files
  List<File> newProofDocumentFiles = [];   // To hold newly selected proof document files


  List<String> deletedCertificateUrls = []; // List to track certificate URLs to delete
  List<String> deletedProofDocumentUrls = []; // List to track proof document URLs to delete
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  if (onlineCourseType == 'past') // Skills acquired only for past courses
                    TextField(
                      controller: editSkillsAcquiredController,
                      decoration: const InputDecoration(labelText: 'Skills Acquired (Optional)'),
                      maxLines: 2,
                    ),
                  ...
                  if (onlineCourseType == 'past')
                    Column(
                      ...
                      children: [
                        ...
                        ...List<Widget>.generate(certificateFileNames.length, (index) {
                          String fileName = certificateFileNames[index];
                          return Padding(
                            padding: const EdgeInsets.symmetric(vertical: 4.0),
                            child: Row(
                              children: [
                                Expanded(child: Text(fileName)), // Display file name or URL for existing
                                IconButton(
                                  icon: const Icon(Icons.delete, color: Colors.red),
                                  onPressed: () {
                                    String urlToDelete = currentCertificateUrls[index]; // Use current URL for deletion
                                    deletedCertificateUrls.add(urlToDelete);
                                    setState(() {
                                      certificateFileNames.removeAt(index);
                                      currentCertificateUrls.removeAt(index); // Keep current URLs list updated
                                    });
                                  },
                                ),
                              ],
                        ...
                        ElevatedButton(
                          onPressed: () async {
                            FilePickerResult? result = await FilePicker.platform.pickFiles(
                              type: FileType.custom,
                              allowedExtensions: ['pdf', 'jpg', 'png', 'jpeg'], // Example extensions
                              allowMultiple: true,
                            );
                            if (result != null) {
                              setState(() {
                                newCertificateFiles.addAll(result.files.map((e) => File(e.path!)));
                                certificateFileNames.addAll(result.files.map((e) => e.name)); // Display file names
                              });
                            }
                          },
                          style: ElevatedButton.styleFrom(backgroundColor: Pallet.headingColor),
                          child: const Text('Add Certificates', style: TextStyle(color: Colors.white)),
                        ),
                      ],
                    ),
                  if (onlineCourseType == 'past') const SizedBox(height: 10),
                  ...
                  if (onlineCourseType == 'current')
                    const Text("Proof Documents:", style: TextStyle(fontWeight: FontWeight.bold)),
                  if (onlineCourseType == 'current')
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        // Display current proof document files (or URLs initially)
                        ...List<Widget>.generate(proofDocumentFileNames.length, (index) {
                            ...
                            child: Row(
                              children: [
                                ...
                                IconButton(
                                  icon: const Icon(Icons.delete, color: Colors.red),
                                  onPressed: () {
                                    String urlToDelete = currentProofDocumentUrls[index]; // Use current URL for deletion
                                    deletedProofDocumentUrls.add(urlToDelete);
                                    setState(() {
                                      proofDocumentFileNames.removeAt(index);
                                      currentProofDocumentUrls.removeAt(index); // Keep current URLs list updated
                                    });
                                  },
                                ),
                              ],
                        ...
                        ElevatedButton(
                          onPressed: () async {
                            ...
                            if (result != null) {
                              setState(() {
                                newProofDocumentFiles.addAll(result.files.map((e) => File(e.path!)));
                                proofDocumentFileNames.addAll(result.files.map((e) => e.name)); // Display file names
                              });
                            }
                          },
                          style: ElevatedButton.styleFrom(backgroundColor: Pallet.headingColor),
                  ...
                  if (onlineCourseType == 'current') const SizedBox(height: 10),
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  String updatedCourseName = editCourseNameController.text.trim();
                  String updatedPlatformName = editPlatformNameController.text.trim();
                  String updatedDuration = editDurationController.text.trim();
                  String updatedSkillsAcquired = editSkillsAcquiredController.text.trim();
                  ...
                  if (newCertificateFiles.isNotEmpty) {
                    uploadedCertificateUrls = await _uploadMediaFilesToStorage(
                  ...
                  if (newProofDocumentFiles.isNotEmpty) {
                    uploadedProofDocumentUrls = await _uploadMediaFilesToStorage(
                      studentId: auth.currentUser!.uid,
                  ...
                  if (onlineCourseType == 'past') {
                    ...
                    updateData['certificateUrl'] = [...currentCertificateUrls, ...uploadedCertificateUrls]; // Combine old and new URLs
                  } else if (onlineCourseType == 'current') {
                    updateData['proofDocumentUrl'] = [...currentProofDocumentUrls, ...uploadedProofDocumentUrls]; // Combine old and new URLs
                  }


                  String studentId = auth.currentUser!.uid;
                  String oldDocId = currentCourseName; // Use courseName as doc ID
                  ...
                  await _deleteMediaFilesFromStorage(deletedProofDocumentUrls);
...
Future<List<String>> _uploadMediaFilesToStorage({
  ...
  for (File file in files) {
    String fileName = file.path.split('/').last;
    Reference storageReference = storage.ref().child('$folderPath/$studentId/$fileName');
    try {
      ...
      print('File upload error: $fileName - $e');
