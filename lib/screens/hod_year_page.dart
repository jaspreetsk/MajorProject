...
import 'package:academiax/screens/hod_section_page.dart';
...
class HodYearPage extends StatefulWidget {
  ...
  const HodYearPage(
      {super.key,
      required this.year,
      required this.departmentName}); // Receive departmentName
    ...
    try {
      ...
      if (widget.year == "Year I") {
        studentSnapshot = await FirebaseFirestore.instance
            .collection('Students')
            .where('department',
                isEqualTo: widget.departmentName) // Filter by department
          ...
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => StudentListPage(
                year: widget.year,
                section: sectionName,
                departmentName: widget.departmentName, // Pass departmentName
              ),
            ),
          );
        },
